import DisplayProfile from "./DisplayProfile";

function Phone() {
  return (
    <div className="rounded-[55px] border border-main-gray w-[307px] h-[631px] relative">
      <MobileLine />
      <DisplayProfile
        className="z-20 absolute top-0 translate-y-[60px] w-[237px] left-1/2 -translate-x-1/2"
        isPreview={false}
      />
    </div>
  );
}
export default Phone;

const MobileLine = () => {
  return (
    <svg
      className="absolute top-0 left-1/2  -translate-x-1/2 translate-y-[10px]"
      width="286"
      height="612"
      viewBox="0 0 286 612"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M1 45.5C1 20.9233 20.9233 1 45.5 1H69.5C75.8513 1 81 6.14873 81 12.5C81 20.5081 87.4919 27 95.5 27H190.5C198.508 27 205 20.5081 205 12.5C205 6.14873 210.149 1 216.5 1H240.5C265.077 1 285 20.9233 285 45.5V566.5C285 591.077 265.077 611 240.5 611H45.5C20.9233 611 1 591.077 1 566.5V45.5Z"
        fill="white"
        stroke="#737373"
      />
    </svg>
  );
};
